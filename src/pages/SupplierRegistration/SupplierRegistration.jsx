...
const SupplierRegistration = () => {
    ...
    const handleContinue = async () => {
        ...
        if (stepIndex === 0) {
            const { shopName, hasGst, gstNumber, panNumber } = registrationData.businessDetails;
            ...
            // PAN Validation - mandatory for all suppliers
            if (!panNumber || panNumber.length !== 10) return toast.error("Enter valid 10-digit PAN number");
            const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
            if (!panRegex.test(panNumber)) return toast.error("Invalid PAN format (e.g. ABCDE1234F)");
        }
                ...
                <AnimatePresence mode="wait">
                    ...
                    {stepIndex === 0 && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key="b" className="p-5">
                            ...
                            {/* PAN Number - Required for all suppliers */}
                            <div className="mt-6">
                                <label className="text-gray-700 font-bold text-xs mb-2 block flex items-center gap-1.5">
                                    <Hash size={14} className="text-[#7C3AED]" />
                                    PAN Card Number <span className="text-red-400 text-[10px]">*Required</span>
                                </label>
                                <div className="relative">
                                    <input 
                                        placeholder="ABCDE1234F"
                                        maxLength={10}
                                        value={registrationData.businessDetails.panNumber}
                                        onChange={(e) => {
                                            const val = e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '');
                                            setRegistrationData(prev => ({ ...prev, businessDetails: { ...prev.businessDetails, panNumber: val } }));
                                        }}
                                        className={`w-full bg-gray-50 border rounded-lg p-3.5 text-sm font-bold tracking-widest focus:outline-none focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/10 transition-all ${
                                            registrationData.businessDetails.panNumber.length === 10 
                                                ? (/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(registrationData.businessDetails.panNumber) ? 'border-green-300 bg-green-50/30' : 'border-red-300 bg-red-50/30')
                                                : 'border-gray-100'
                                        }`}
                                    />
                                    <div className="absolute right-4 top-1/2 -translate-y-1/2">
                                        {registrationData.businessDetails.panNumber.length === 10 && (
                                            /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(registrationData.businessDetails.panNumber)
                                                ? <Check size={18} className="text-green-500" />
                                                : <AlertCircle size={18} className="text-red-500" />
                                        )}
                                    </div>
                                </div>
                                {registrationData.businessDetails.panNumber.length > 0 && registrationData.businessDetails.panNumber.length < 10 && (
                                    <p className="text-[10px] text-gray-400 mt-1.5 ml-1 font-medium">Format: 5 letters + 4 digits + 1 letter (e.g. ABCDE1234F)</p>
                                )}
                                {registrationData.businessDetails.panNumber.length === 10 && !/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(registrationData.businessDetails.panNumber) && (
                                    <p className="text-[10px] text-red-500 mt-1.5 ml-1 font-bold flex items-center gap-1"><AlertCircle size={10} /> Invalid PAN format</p>
                                )}
                            </div>
                        </motion.div>
                    )}
