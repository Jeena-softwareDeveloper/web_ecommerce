...
const Checkout = () => {
    ...
    // Redux State
    const { cartItems } = useSelector(state => state.wearCart);
    const { token, userInfo } = useSelector(state => state.auth);
    const { wearConfig, paymentKeys } = useSelector(state => state.config);
    const { loader: orderLoading } = useSelector(state => state.order || {});
    ...
    // Fetch RTO Risk Score and Dynamic Shipping Rate when address is selected
    useEffect(() => {
        if (selectedAddress?.phone) {
            setIsRtoChecking(true);
            api.get(`/wear/orders/order/rto-risk/${selectedAddress.phone}`, { skipToast: true })
                .then(res => setRtoRisk(res.data.data?.risk_score || 0))
                .catch(() => setRtoRisk(0))
                .finally(() => setIsRtoChecking(false));
        }
        if (selectedAddress?.pincode) {
            setShippingError(null);
            setIsShippingLoading(true);
            api.get(`/wear/orders/order/shipping-rate/${selectedAddress.pincode}?weight=0.5&cod=${paymentMethod === 'COD' ? 1 : 0}`, { skipToast: true })
                .then(res => setDynamicShipping(res.data.data?.rate || null))
                .catch(err => {
                    setDynamicShipping(null);
                    if (err.response?.status === 400) {
                        setShippingError(err.response?.data?.error || "Courier not serviceable");
                    }
                })
    ...
    }, [addressSuccess, addressError, dispatch, closeAddressModal]);
    ...
    const getTieredPrice = (product, qty) => {
        ...
        return matchingTier ? (matchingTier.finalPrice || matchingTier.unitPrice || matchingTier.price || basePrice) : basePrice;
    };
    ...
    const { calculateTotal, calculateOriginalTotal, savings, isAnyOutOfStock, checkoutItems } = useMemo(() => {
        ...
        checkoutItems.forEach(item => {
            ...
            // Check stock (For bulk products, allow ordering more than stock if stock > 0)
            const isBulk = product.isBulkOnly || (product.variants || []).some(v => v.priceTiers?.length > 0);
            if (variant && variant.stock < item.quantity) {
                if (!isBulk || variant.stock <= 0) {
                    anyOutOfStock = true;
                }
            }
        ...
        return {
            calculateTotal: Math.round(total),
            calculateOriginalTotal: Math.round(originalTotal),
            savings: Math.round(originalTotal - total),
    ...
    const deliveryChargeAmt = dynamicShipping !== null ? Math.round(dynamicShipping) : (selectedAddress ? 0 : (wearConfig?.delivery_charge));
    const isFreeDelivery = (wearConfig?.is_free_delivery_enabled !== false) && (calculateTotal >= deliveryChargeThreshold);
    const appliedDeliveryCharge = isFreeDelivery ? 0 : deliveryChargeAmt;
    const finalTotal = calculateTotal + appliedDeliveryCharge;

    const isHighRtoRisk = rtoRisk > 0.6; // High risk threshold
    const canCod = !isHighRtoRisk &&
        wearConfig?.is_cod_enabled !== false &&
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            if (cashfreeAppId) {
                ...
                // Let Cashfree handle the payment method selection UI natively
                                ...
                                <motion.div key="address" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                                            ...
                                            return (
                                                <div key={item._id} className={`p-3 flex flex-row ${idx !== checkoutItems.length - 1 ? 'border-b border-gray-50' : ''}`}>
                                                    ...
                                                    <div className="flex-1 ml-4 flex flex-col justify-between py-0.5">
                                                        <div>
                                                            <p className="text-gray-700 font-bold text-[11px] leading-tight mb-1 line-clamp-2">{finalName}</p>
                                    ...
                                    <div className="hidden md:block">
                                        <button
                                            ...
                                            className="w-full mt-6 py-4 bg-[#e11955] text-white rounded-lg font-bold uppercase tracking-widest shadow-lg shadow-rose-100 flex items-center justify-center hover:opacity-90 transition-all"
                                        >
                                            <span>{!selectedAddress ? 'Add Address' : shippingError ? 'Not Serviceable' : 'Continue to Payment'}</span><ArrowRight size={16} className="ml-2" />
                                        </button>
                                    </div>
                                ...
                                <motion.div key="payment" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                                    ...
                                    <div className={`bg-white mb-2 border-t border-b border-gray-100 md:rounded-2xl md:border md:shadow-sm md:overflow-hidden ${!canCod ? 'opacity-50' : ''}`}>
                                        {renderPaymentMethod(
                                            ...
                                            null,
                                            isRtoChecking
                                                ? <span className="text-[10px] text-gray-400 font-bold flex items-center gap-1"><RefreshCw size={10} className="animate-spin" /> Checking...</span>
                                                : !canCod
                                                    ? <span className="text-[10px] text-red-500 font-bold flex items-center gap-1">
                                                        {isHighRtoRisk ? 'Not available for your location' :
                                            ...
                                            !canCod || isRtoChecking
                                        )}
                                    </div>

                                    {/* Online Option */}
                                    <div className={`bg-white mb-4 border-t border-b border-gray-100 md:rounded-2xl md:border md:shadow-sm md:overflow-hidden ${paymentMethod === 'ONLINE' ? 'bg-rose-50/10 border-rose-100' : ''}`}>
                                        {renderPaymentMethod(
                                            'ONLINE',
                                            'Pay Online (UPI, Cards, Wallet)',
                                            null,
                                            <div className="flex items-center">
                            ...
                            <div className="space-y-4 mb-6">
                                ...
                                <div className="flex justify-between items-center">
                                    <div className="flex flex-col">
                                        ...
                                        {shippingError && (
                                            <span className="text-[8px] text-amber-600 font-bold uppercase tracking-widest flex items-center gap-1 mt-0.5">
                                                <AlertCircle size={10} /> {shippingError} (Standard Applied)
                                            </span>
                                        )}
                                    </div>
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex flex-row items-center justify-between p-3 shadow-[0_-10px_20px_rgba(0,0,0,0.1)] z-40">
                ...
                {step === 1 ? (
                    <button
                        ...
                        className={`px-10 h-12 rounded-lg flex items-center justify-center shadow-lg active:scale-95 transition-all ${shippingError || isShippingLoading ? 'bg-gray-300' : 'bg-[#e11955] shadow-rose-100'}`}
                    >
                        {isShippingLoading ? (
                            <RefreshCw size={18} className="animate-spin text-white" />
                        ) : (
                            <span className="text-white font-bold text-[14px] uppercase tracking-widest">{!selectedAddress ? 'Add Address' : shippingError ? 'Not Serviceable' : 'Continue'}</span>
                        )}
                    </button>
                ) : (
                    <button
                        disabled={isAnyOutOfStock || orderLoading || !!shippingError}
                        onClick={handlePlaceOrder}
                        className={`px-8 h-12 rounded-lg flex items-center justify-center shadow-lg active:scale-95 transition-all ${isAnyOutOfStock || !!shippingError ? 'bg-gray-300' : 'bg-[#e11955] shadow-rose-100'}`}
                    >
                        {orderLoading ? (
                            <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                        ) : (
                                ...
                                <div className="p-8 overflow-y-auto no-scrollbar space-y-6 flex-1">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-1.5">
                                            ...
                                            <input readOnly required value={newAddress.city} type="text" className="w-full bg-blue-50/30 border border-blue-100/50 rounded-lg p-4 font-bold text-gray-800 cursor-not-allowed" placeholder="City" />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">State</label>
                                            <input readOnly required value={newAddress.state} type="text" className="w-full bg-blue-50/30 border border-blue-100/50 rounded-lg p-4 font-bold text-gray-800 cursor-not-allowed" placeholder="State" />
                                        </div>
                                    </div>
