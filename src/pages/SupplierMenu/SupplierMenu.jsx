...
const SupplierMenu = () => {
    ...
    useEffect(() => {
        // Only fetch if not already loaded — Dashboard already fetches this on mount
        if (!navMenu.supplier || navMenu.supplier.length === 0) {
            dispatch(get_nav_menu('supplier'));
        }
    }, [dispatch, navMenu.supplier]);
                ...
                <div className="bg-white px-5 py-6 flex items-center mb-4">
                    ...
                    <div>
                        <h2 className="text-[18px] font-black text-gray-900 leading-tight">
                            {supplierData?.shopName ? supplierData.shopName : <span className="text-red-500">Shop Name Missing</span>}
                        </h2>
