...
import { get_inventory_alerts } from '../../store/reducers/supplierStockReducer';
...
const SupplierDashboard = () => {
    ...
    const { alerts, alertCount } = useSelector(state => state.supplierStock);
    const { navMenu } = useSelector(state => state.config);

    useEffect(() => {
        dispatch(get_supplier_dashboard_stats());
        dispatch(get_notifications({ read: false, limit: 1 }));
        dispatch(get_inventory_alerts());
        if (!navMenu.supplier || navMenu.supplier.length === 0) {
    ...
    return (
        <div className="h-screen h-[100dvh] flex flex-col overflow-hidden bg-transparent">
            {/* FIXED TOP CONTAINER */}
            <div className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 shadow-sm max-w-md mx-auto border-x">
                    ...
                    <div className="flex items-center flex-1">
                        ...
                        <div className="flex-1">
                            <h1 className="text-lg font-black text-gray-900 leading-tight">
                                {supplierData?.shopName || <span className="text-red-500">Shop Name Missing</span>}
                            </h1>
            ...
            <div className="p-4 max-w-md mx-auto space-y-6">
                ...
                {alertCount > 0 && (
                    <div 
                        ...
                        className="mb-6 p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-center gap-3 cursor-pointer active:scale-98 transition-all"
                    >
                        <div className="w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center shrink-0">
                            <LucideIcons.AlertTriangle className="text-amber-600" size={20} />
                        </div>
                        <div className="flex-1">
                            <p className="text-sm font-bold text-amber-900">Inventory Alerts ({alertCount})</p>
                            <p className="text-[11px] text-amber-700 mt-0.5">
                                {alerts.filter(a => a.type === 'out_of_stock').length} Out of Stock · {alerts.filter(a => a.type === 'low_stock').length} Low Stock
                            </p>
                        </div>
                        <LucideIcons.ChevronRight className="text-amber-400" size={18} />
                    </div>
                )}

                {/* PERFORMANCE AND SALES STATS GRID */}
                <div className="grid grid-cols-6 lg:grid-cols-5 gap-3 lg:gap-4 mb-8">
                    <StatsCard
                        title="To Confirm"
                        ...
                        className="col-span-2 lg:col-span-1 cursor-pointer active:bg-amber-100/50"
                    />
                    <StatsCard
                        title="To Ship"
                        ...
                        onClick={() => navigate('/supplier-orders')}
                        className="col-span-2 lg:col-span-1 cursor-pointer active:bg-blue-100/50"
                    />
                    <StatsCard
                        title="Returns"
                        ...
                        className="col-span-2 lg:col-span-1 cursor-pointer active:bg-rose-100/50"
                    />
                    <StatsCard
                        title="Total Sales"
                        ...
                        className="col-span-3 lg:col-span-1 cursor-pointer active:bg-emerald-100/50"
                    />
                    <StatsCard
                        title="Total Orders"
                        ...
                        className="col-span-3 lg:col-span-1 cursor-pointer active:bg-purple-100/50"
                    />
                </div>
                ...
                >
                    <div className="space-y-3">
                        ...
                        <LinkItem 
                            ...
                            subtitle="Keep 100% of your earnings on Jeenora"
                            icon={<LucideIcons.Percent />} 
                            iconBg="bg-blue-50" 
