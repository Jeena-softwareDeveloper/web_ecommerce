...
const AICustomerSupport = () => {
    ...
    const startChatSession = async (lang, cat) => {
        setStep(2);
        setIsTyping(true);

        try {
            const { data } = await apiClient.post('/wear/home/customer/ai/support', {
                message: `[SYSTEM: Welcome the user. Language: ${lang}, Issue: ${cat}. This is the start of the chat.]`,
                chatHistory: [],
                agentName: agentName
            });

            setMessages([
                { role: 'assistant', text: data.replyText, type: 'text' }
            ]);
        } catch (error) {
            setMessages([
                { role: 'assistant', text: `Hi! I am ${agentName}, your Support Executive. How can I assist you with ${cat}?`, type: 'text' }
            ]);
        } finally {
            setIsTyping(false);
        }
    ...
    return (
        <div className="fixed inset-0 h-screen h-[100dvh] bg-gray-50 flex flex-col font-sans overflow-hidden">
            {/* Header */}
            ...
            {step === 0 && (
                <div className="flex-1 max-w-lg w-full mx-auto p-6 flex flex-col justify-center items-center">
            ...
            {step === 2 && (
                <div className="flex-1 max-w-3xl w-full mx-auto p-4 flex flex-col overflow-hidden">
                    <div className="flex-1 overflow-y-auto overscroll-contain custom-scrollbar flex flex-col gap-5 pb-4 mt-2">
