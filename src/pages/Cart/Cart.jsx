                        ...
                        <div className="md:col-span-2 space-y-2 md:space-y-4">
                            <AnimatePresence mode="popLayout">
                                {cartItems.map((item) => {
                                    ...
                                    const variant = (product.variants || []).find(v => v.size === item.size) || product.variants?.[0];
                                    const isBulk = product.isBulkOnly || (product.variants || []).some(v => v.priceTiers?.length > 0);
                                    const isOutOfStock = variant && variant.stock < item.quantity && (!isBulk || variant.stock <= 0);
                                    const tieredPrice = getTieredPrice(product, item.quantity);
                                    const mrp = variant?.mrp || variant?.listingPrice || product.price || 0;
                                    const itemSavings = Math.round((mrp - tieredPrice) * item.quantity);
                                                ...
                                                <div className="flex-1 ml-4 pr-6 flex flex-col relative">
                                                    ...
                                                    <div className="flex flex-wrap items-center mt-auto gap-2">
                                                        ...
                                                        {isOutOfStock ? (
                                                            ...
                                                            <div className="flex items-center bg-indigo-50 border border-indigo-100 px-2 py-1 rounded-lg">
                                                                <Package size={10} className="text-indigo-600" />
                                                                <span className="text-[9px] text-indigo-600 font-bold ml-1 uppercase">Bulk Manufacturing Order</span>
                                                            </div>
                                                        ) : (
                                                            <div className="flex items-center bg-rose-50/50 border border-red-100/50 px-2 py-1 rounded-lg">
                                                                ...
                                                                <span className="text-[9px] text-[#e11955] font-bold ml-1 uppercase">Ready to Ship</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                ...
                                <div className="flex mb-6">
                                    ...
                                    <div className="flex-1 ml-4 flex flex-col justify-center">
                                        <span className="text-[12px] font-bold text-gray-800 mb-1 line-clamp-2">
                                            {editingItem.productId?.name || editingItem.productId?.productName}
                                        </span>
